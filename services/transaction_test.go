...
func TestTransaction(t *testing.T){
	...
	account1, success:= GetAccount(accountId1)
	require.True(t, success)

	account2, success := GetAccount(accountId2)
	require.True(t, success)
	...
	for i:=0; i<routines; i++{
		go func(){
			transaction, res, isSuccessful :=DoubleEntry(accountId1, accountId2, decimal.NewFromInt32(amount))
